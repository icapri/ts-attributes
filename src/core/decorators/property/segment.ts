    ...
    Object.defineProperty(target, propertyKey, {
      set: (nextValue: any) => {
        if (!Validator.isNullOrUndefined(nextValue) && !Validator.isNumber(nextValue)) {
          throw new Error(`Value of '${propertyKey}' should be a number. (${target.constructor.name})`);
        }
        ...
        if (including && (nextValue < from || nextValue > to)) {
          throw new Error(`Value of '${propertyKey}' should be a number between ${from} and ${to} incl. them. (${target.constructor.name})`);
        }

        if (!including && (nextValue <= from || nextValue >= to)) {
          throw new Error(`Value of '${propertyKey}' should be a number between ${from} and ${to}. (${target.constructor.name})`);
        }
